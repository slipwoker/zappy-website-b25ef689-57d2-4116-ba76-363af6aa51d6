    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (contactForm) {
            ...
            function validatePhone(phone) {
                // Allow optional country code, numbers, spaces, dashes, parentheses
            ...
            function validateField(input) {
                ...
                if (pattern) {
                    ...
                    if (!regex.test(value)) {
                        const patternMsg = input.getAttribute('data-pattern-error') || 'Please match the required format';
            ...
            contactForm.addEventListener('input', function(e) {
                const input = e.target.closest('input, textarea, select');
                if (input && input.name && input.classList.contains(errorClass)) {
                    validateField(input);
                }
            });
            ...
            contactForm.addEventListener('submit', function(e) {
                ...
                if (isValid) {
                    ...
                    // Dispatch custom event with form data for post-processing
                    ...
                    const firstInvalid = contactForm.querySelector('.' + errorClass);
                    if (firstInvalid) {
                        firstInvalid.focus();
                    }
                }
        ...
        if (animatedElements.length > 0) {
            function isElementInViewport(el, offset) {
                ...
                return (
                    rect.top <= windowHeight - elementOffset &&
                    rect.bottom >= 0 - elementOffset
                );
            }
                    ...
                    mutation.addedNodes.forEach(function(node) {
                        if (node.nodeType === 1) {
                            if (node.matches && node.matches('.fade-in, .fade-in-up, .fade-in-left, .fade-in-right, .animate-on-scroll')) {
                                if (isElementInViewport(node, 50)) {
                                    node.classList.add('visible', 'animated');
                                }
                            }
                            // Check children
                            if (node.querySelectorAll) {
                                const children = node.querySelectorAll('.fade-in, .fade-in-up, .fade-in-left, .fade-in-right, .animate-on-scroll');
                                children.forEach(function(child) {
                                    if (isElementInViewport(child, 50)) {
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'b25ef689-57d2-4116-ba76-363af6aa51d6',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'b25ef689-57d2-4116-ba76-363af6aa51d6';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'b25ef689-57d2-4116-ba76-363af6aa51d6';
